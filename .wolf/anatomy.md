...
- `any-thoughts-on-why-graceful-cook.md` — Fix: WASM blank page — missing DI registrations (~395 tok)
...
- `tour.md` — Tour Authoring Skill (~3071 tok)
...
- `tour-authoring-guide.md` — Tour Authoring Guide (~5216 tok)
...
- `TourData.cs` — AXAML-instantiable content class for a tour definition. Converted to an immutable <see cref="TourDef (~345 tok)
- `TourDefinition.cs` — A named, ordered sequence of <see cref="TourSegment"/> references forming a complete walkthrough exp (~779 tok)
- `TourPlacement.cs` — Preferred position of the tour card relative to its target element. The presentation layer uses this (~139 tok)
- `TourProgress.cs` — Tracks the user's progress through a running or completed tour. The only mutable entity in the tour (~558 tok)
...
- `Step2DatabaseViewModel.cs` — Step 2 — choose the database folder, confirm/edit the database filename, and choose the backup folde (~1705 tok)

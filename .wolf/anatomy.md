...
> Auto-maintained by OpenWolf. Last scanned: 2026-03-30T01:23:10.569Z
...
- `CheckoutServiceTests.cs` — Integration tests for <see cref="CheckoutService"/>. <para>Each test uses an isolated temporary dire (~11452 tok)
...
- `MainWindow.axaml` — Declares applied (~10775 tok)
- `MainWindow.axaml.cs` — Called whenever the window is about to close — whether via Files → Exit or the title-bar X. Cancels (~11311 tok)
...
- `CheckoutService.cs` — Manages the checkout / save lifecycle for every database the app opens. <para><b>Core concept:</b> W (~8256 tok)

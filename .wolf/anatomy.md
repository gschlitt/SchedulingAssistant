...
- `settings.local.json` — /*.cs)", (~2259 tok)
...
- `MainWindow.axaml.cs` — Called whenever the window is about to close — whether via Files → Exit or the title-bar X. All shutdown logic (e.g. backup, save-state) belongs he... (~7984 tok)
- `Program.cs` — Application entry point (~429 tok)
...
- `SectionPropertyRepository.cs` — Returns all property values of the given type, ordered by <see cref="SectionPropertyValue.SortOrder"/> ascending, then alphabetically by name as a ... (~1028 tok)
- `SectionRepository.cs` — Returns all sections for the given course across all semesters, ordered by section code. (~1840 tok)
- `SemesterRepository.cs` — SemesterRepository: GetAll, GetByAcademicYear, Insert, Update + 2 more (~836 tok)
- `SubjectRepository.cs` — Returns true if any courses belong to this subject. (~1167 tok)

## src/SchedulingAssistant/Exceptions/

- `DatabaseCorruptException.cs` — Thrown by <see cref="App.InitializeServices"/> when <c>PRAGMA integrity_check</c> reports structural problems in the main database file. Catching t... (~222 tok)

...
- `MainWindow.axaml.cs` — Minimum time the loading curtain stays up, to avoid a flicker on fast loads. (~18223 tok)
...
- `CheckoutService.cs` — Manages the checkout / save lifecycle for every database the app opens. <para><b>Write-access mode:< (~20544 tok)
...
- `WriteLockService.cs` — Manages a file-based write lock that prevents two instances of the app from writing to the same SQLi (~10887 tok)

## src/SchedulingAssistant/ViewModels/

- `MainWindowViewModel.cs` — The permanent left-panel section list. Held for app lifetime. (~9213 tok)
...
- `MeetingListView.axaml` (~11993 tok)

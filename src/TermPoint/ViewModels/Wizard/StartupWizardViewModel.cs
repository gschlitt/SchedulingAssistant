...
using TermPoint.ViewModels.Wizard.Steps;
...
///   0  — Welcome
///   1  — License Agreement
///   2  — Existing-DB check (Step1a)
///   3  — Institution
///   4  — Database location + filename
///   5  — TpConfig import / ExitNow
///   6  — Campuses
///   7  — Legal start times
///   8  — Block patterns
...
public partial class StartupWizardViewModel : ViewModelBase
{
    ...
    private async Task WriteDbRecordsAsync()
    {
        ...
        // --- Import path: campuses and block patterns from .tpconfig ---
        if (importedCfg is not null)
        {
            var campusRepo = _services.Campuses();

            // Insert campuses in sort order.
        ...
        if (importedCfg?.BlockLengths is { Count: > 0 } importBlockLengths)
        {
            var seedData = importBlockLengths
                ...
                .ToList();
            TermPoint.Data.SeedData.SeedWizardLegalStartTimes(db.Connection, ay.Id, seedData);

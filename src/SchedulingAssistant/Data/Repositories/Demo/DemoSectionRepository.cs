...
/// the in-memory list so that edits, adds, copies, and deletes are reflected
/// in the schedule grid within the browser session.  Changes are lost on page reload.
/// </summary>
public class DemoSectionRepository : ISectionRepository
{
    // Mutable list initialised once from the static demo data at construction time.
    // Because this class is registered as a singleton, the same list is shared
    // across all reads and writes for the lifetime of the browser session.
